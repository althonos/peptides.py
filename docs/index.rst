...
- Amino-acid Statistics:
  ...
  - Longuest run of identical amino-acids
...
- Biological properties:
  ...
  - Sequence validation comparing several metrics against a reference distribution
    computed from the `SwissProt <https://www.expasy.org/resources/uniprotkb-swiss-prot>`_
    database.
...
``peptides`` is a pure Python package available for all modern Python (3.6+).

Run ``pip install peptides`` in a shell to download the latest release, or have
a look at the :doc:`Installation page <guide/install>` to find other ways to install
``peptides.py``.
